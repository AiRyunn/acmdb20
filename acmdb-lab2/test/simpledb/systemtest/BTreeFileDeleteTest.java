...
public class BTreeFileDeleteTest extends SimpleDbTestBase {
    ...
    public void testRedistributeLeafPages() throws Exception {
        ...
        BTreeFile twoLeafPageFile = BTreeUtility.createRandomBTreeFile(2, 600, null, null, 0);
        ...
        while (it.hasNext() && count < 49) {
            ...
            BTreeLeafPage p = (BTreeLeafPage) Database.getBufferPool().getPage(tid, pid, Permissions.READ_ONLY);
        ...
        BTreeLeafPage rightSibling = (BTreeLeafPage) Database.getBufferPool().getPage(tid, rightSiblingId,
    ...
    public void testMergeLeafPages() throws Exception {
        ...
        BTreeFile threeLeafPageFile = BTreeUtility.createRandomBTreeFile(2, 1005, null, null, 0);
        ...
        BTreeRootPtrPage rootPtr = (BTreeRootPtrPage) Database.getBufferPool().getPage(tid, rootPtrId,
                Permissions.READ_ONLY);
        BTreeInternalPage root = (BTreeInternalPage) Database.getBufferPool().getPage(tid, rootPtr.getRootId(),
                Permissions.READ_ONLY);
    ...
    public void testDeleteRootPage() throws Exception {
        ...
        BTreeFile twoLeafPageFile = BTreeUtility.createRandomBTreeFile(2, 503, null, null, 0);
        ...
        BTreeLeafPage root = (BTreeLeafPage) Database.getBufferPool().getPage(tid, rootPtr.getRootId(),
                Permissions.READ_ONLY);
    ...
    public void testReuseDeletedPages() throws Exception {
        ...
        for (int i = 0; i < 502; ++i) {
            Database.getBufferPool().insertTuple(tid, threeLeafPageFile.getId(), BTreeUtility.getBTreeTuple(i, 2));
        }
    ...
    public void testRedistributeInternalPages() throws Exception {
        ...
        BTreeFile bf = BTreeUtility.createRandomBTreeFile(2, 302204, null, null, 0);
        ...
        BTreeRootPtrPage rootPtr = (BTreeRootPtrPage) Database.getBufferPool().getPage(tid,
                BTreeRootPtrPage.getId(bf.getId()), Permissions.READ_ONLY);
        BTreeInternalPage root = (BTreeInternalPage) Database.getBufferPool().getPage(tid, rootPtr.getRootId(),
                Permissions.READ_ONLY);
        assertEquals(502, root.getNumEmptySlots());

        BTreeEntry rootEntry = root.iterator().next();
        BTreeInternalPage leftChild = (BTreeInternalPage) Database.getBufferPool().getPage(tid,
                rootEntry.getLeftChild(), Permissions.READ_ONLY);
        BTreeInternalPage rightChild = (BTreeInternalPage) Database.getBufferPool().getPage(tid,
                rootEntry.getRightChild(), Permissions.READ_ONLY);
        ...
        while (it.hasNext() && count < 49 * 502 + 1) {
            BTreeLeafPage leaf = (BTreeLeafPage) Database.getBufferPool().getPage(tid, it.next().getLeftChild(),
                    Permissions.READ_ONLY);
    ...
    @Test
    public void testRedistributeInternalPages2() throws Exception {
        // This should create a B+ tree with two nodes in the second tier
        // and 602 nodes in the third tier
        BTreeFile bf = BTreeUtility.createRandomBTreeFile(2, 302203, null, null, 0);
        ...
        int count = 0;
        // bring the right internal page to minimum occupancy
        while (it.hasNext() && count < 50 * 502 + 1) {
            BTreeLeafPage leaf = (BTreeLeafPage) Database.getBufferPool().getPage(tid, it.next().getLeftChild(),
                    Permissions.READ_ONLY);
            Tuple t = leaf.iterator().next();
            Database.getBufferPool().deleteTuple(tid, t);
        ...
        //     count++;
        // }
        BTreeChecker.checkRep(bf, tid, new HashMap<PageId, Page>(), false);
    }
    ...
    public void testDeleteInternalPages() throws Exception {
        ...
        // 251*124 + 1 = 31125)
        BTreeFile bigFile = BTreeUtility.createRandomBTreeFile(2, 31125, null, null, 0);
        ...
        leftChild = (BTreeInternalPage) Database.getBufferPool().getPage(tid, e.getLeftChild(), Permissions.READ_ONLY);
        rightChild = (BTreeInternalPage) Database.getBufferPool().getPage(tid, e.getRightChild(),
                Permissions.READ_ONLY);
        assertEquals(0, leftChild.getNumEmptySlots());
        ...
        rootPtr = (BTreeRootPtrPage) Database.getBufferPool().getPage(tid, rootPtrId, Permissions.READ_ONLY);
        assertTrue(rootPtr.getRootId().pgcateg() == BTreePageId.INTERNAL);
        root = (BTreeInternalPage) Database.getBufferPool().getPage(tid, rootPtr.getRootId(), Permissions.READ_ONLY);
