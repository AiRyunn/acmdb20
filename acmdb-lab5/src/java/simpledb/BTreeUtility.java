...
public class BTreeUtility {
    ...
    public static BTreeFile createRandomBTreeFile(int columns, int rows, int maxValue,
            Map<Integer, Integer> columnSpecification, ArrayList<ArrayList<Integer>> tuples, int keyField)
            throws IOException, DbException, TransactionAbortedException {
     ...
     */
    public static ArrayList<BTreeEntry> generateRandomEntries(int numKeys, int tableid, int childPageCategory,
    ...
    public static BTreeLeafPage createRandomLeafPage(BTreePageId pid, int columns, int keyField, int numTuples, int min,
        ...
        byte[] data = BTreeFileEncoder.convertToLeafPage(
                BTreeUtility.generateRandomTuples(columns, numTuples, min, max), BufferPool.getPageSize(), columns,
                typeAr, keyField);
