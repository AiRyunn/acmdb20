...
public class BTreeFile implements DbFile {
    ...
    protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page,
        ...
        int half = (page.getNumTuples() + 1) / 2;
        ...
        // now tuple is the first element of newPage

        BTreePageId oldRightId = page.getRightSiblingId();
        BTreePageId newRightId = newRightPage.getId();
        BTreePageId pageId = page.getId();

        newRightPage.setLeftSiblingId(pageId);
        newRightPage.setRightSiblingId(oldRightId);

        page.setRightSiblingId(newRightPage.getId());
        if (oldRightId != null) {
            BTreeLeafPage oldRightPage = (BTreeLeafPage) this.getPage(tid, dirtypages, oldRightId,
                    Permissions.READ_WRITE);
            oldRightPage.setLeftSiblingId(newRightId);
        }

        BTreeInternalPage parentPage = this.getParentWithEmptySlots(tid, dirtypages, page.getParentId(), field);
        parentPage.insertEntry(new BTreeEntry(tuple.getField(this.keyField), pageId, newRightId));

        newRightPage.setParentId(parentPage.getId());
        return field.compare(Op.LESS_THAN_OR_EQ, tuple.getField(this.keyField)) ? page : newRightPage;
    }
    ...
    protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        // some code goes here
        // TODO: Exception
        BTreeInternalPage newRightPage = (BTreeInternalPage) this.getEmptyPage(tid, dirtypages, BTreePageId.INTERNAL);

        int half = (page.getNumEntries() + 1) / 2;
        ...
        // now entry is the first element of newPage
        newRightPage.deleteKeyAndLeftChild(entry);

        BTreeEntry newEntry = new BTreeEntry(entry.getKey(), page.getId(), newRightPage.getId());
        ...
        return field.compare(Op.LESS_THAN_OR_EQ, newEntry.getKey()) ? page : newRightPage;
    }
    ...
    protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        for (int i = 0; i < count; i++) {
            ...
            leftSibling.deleteKeyAndRightChild(siblingEntry);
            siblingEntry.setLeftChild(leftSibling.getId());
            siblingEntry.setRightChild(page.getId());
    ...
    protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
        ...
        BTreeEntry siblingEntry;
        BTreeEntry entry = page.reverseIterator().next();
        for (int i = 0; i < count; i++) {
            siblingEntry = it.next();
            parent.deleteKeyAndLeftChild(parentEntry);
            parentEntry.setRightChild(siblingEntry.getLeftChild());
            parentEntry.setLeftChild(entry.getRightChild());
            page.insertEntry(parentEntry);
            entry = parentEntry;

            rightSibling.deleteKeyAndLeftChild(siblingEntry);
            siblingEntry.setRightChild(rightSibling.getId());
            siblingEntry.setLeftChild(page.getId());
            parent.insertEntry(siblingEntry);
            parentEntry = siblingEntry;
        }
    ...
    protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage leftPage,
        ...
        if (rightSiblingId != null) {
            BTreeLeafPage rightSibling = (BTreeLeafPage) (this.getPage(tid, dirtypages, rightSiblingId,
                    Permissions.READ_WRITE));
